...
def smrxchanneldata(smrx_path: Path) -> pd.DataFrame:
        ...
        for i in range(rec.MaxChannels()):
            ...
            if chan_type != "DataType.Off":
                ...
                if chan_type == "DataType.Adc":
                    ...
                    fs = -1
                spike2channels.append(
                    dict(
                        chan_name=chan_name,
                        chan_type=chan_type[len("DataType.") :],
    ...
    return pd.DataFrame(
        spike2channels, columns=["chan_name", "chan_type", "physical_channel", "chan_num", "fs", "start_t", "end_t"]
    )
...
def smrxadc2electrophy(smrx_path: Path, channel_nums: list[int | Any] = [...]) -> xr.DataArray:
    ...
    if ... in channel_nums:
        channel_nums = [
            c
            for c in channel_metadata["chan_num"].loc[(channel_metadata["chan_type"] == "Adc")].tolist()
    ...
    if channel_metadata.empty:
        ...
        # return a
        # return
    if (channel_metadata["chan_type"] != "Adc").any():
        raise Exception("Not all channels are Adc")
    fs, start_t, end_t = (
        channel_metadata["fs"].iat[0],
        channel_metadata["start_t"].max(),
        channel_metadata["end_t"].min(),
    )
    ...
    def retrieve_data(slices: list[slice]):
        with sonfile(smrx_path) as rec:
            ...
            for chan_ind in range(channel_slice.start, channel_slice.stop):
                ...
                chunk = np.array(
                    rec.ReadFloats(chan, time_size, divide * slices[1].start + int(np.round(start_t / time_base)))
                )
    ...
    a = xr.DataArray(
        dask_array_from_chunk_function(retrieve_data, (len(channel_nums), n_elems), (1, 10**7), float),
        dims=["channel", "t"],
        name="data",
    )
    a["t"] = np.arange(n_elems) / fs + start_t
    a["t"].attrs["fs"] = fs
    a["chan_num"] = xr.DataArray(channel_metadata["chan_num"].to_numpy(), dims="channel")
